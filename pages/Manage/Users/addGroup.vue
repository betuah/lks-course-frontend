               ...
               >
                  <div v-for="(item, index) in roleItems" :key="item.roles">
                     <v-checkbox
                        ...
                        dense
                        hide-details
                        @change="changeReadPermission(index)"
...
export default {
   ...
   methods: {
      ...
      changePermission(index) {
         ...
         this.changeReadPermission(index);
      },
      changeReadPermission(index) {
         if (
            !this.roleItems[index].create &&
            !this.roleItems[index].update &&
            !this.roleItems[index].delete
         ) {
            this.roleItems[index].selected = false;
         }

         if (!this.roleItems[index].selected) {
            this.roleItems[index].create = false;
            this.roleItems[index].update = false;
            this.roleItems[index].delete = false;
         }
      },
      async getRoles() {
         try {
      ...
      async submitForm() {
         if (this.$refs.form.validate()) {
            ...
            try {
               ...
               const res = await this.$axios.post("/group", data);
               const resData = res.data;

               if (resData.success) {
                  this.showNotif("success", `Success create new group.`);
                  await this.getRoles();
                  this.loading = false;
